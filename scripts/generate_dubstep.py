...
# Placeholder AudioLayer class with debug in get_precomputed_step
class AudioLayer:
    ...
    def get_precomputed_step(self, step_idx):
        ...
        print(f"[AudioLayer] {self.name} step {step_idx} stats: min={step_audio.min():.4f}, max={step_audio.max():.4f}, mean={step_audio.mean():.4f}")
        return step_audio
...
class StepSequencer:
    ...
    def synth_next_step(self):
        ...
        for layer in self.layers:
            ...
            # Normalize step_audio to prevent clipping
            max_val = np.max(np.abs(step_audio))
            if max_val > 0:
                step_audio = step_audio / max_val
            stereo_buffer += step_audio
        # Normalize buffer if needed
        max_val = np.max(np.abs(stereo_buffer))
        if max_val > 1.0:
            stereo_buffer = stereo_buffer / max_val
        print(f"[StepSequencer] Step {self.current_step} mixed buffer stats: min={stereo_buffer.min():.4f}, max={stereo_buffer.max():.4f}, mean={stereo_buffer.mean():.4f}")
...
class ArrangementController:
    ...
    def generate_song(self, section_plan):
        ...
        # Normalize final track with small epsilon to avoid div by zero
        ...
        print(f"[ArrangementController] Final audio stats: min={normalized_song.min():.4f}, max={normalized_song.max():.4f}, mean={normalized_song.mean():.4f}")
...
def main():
    ...
    for layer in layers:
        ...
        layer.precompute_predictions_for_song(64)

    sequencer = StepSequencer(layers, pattern_length=64)
    arrangement = ArrangementController(sequencer, total_steps=64)

    section_plan = [("Full Song", 64)]
    print("Generating song...")
    song_audio = arrangement.generate_song(section_plan)

    print(f"[generate_dubstep] Pre-save audio stats: min={song_audio.min():.4f}, max={song_audio.max():.4f}, mean={song_audio.mean():.4f}")
